...
@section('pages')
    ...
    @include('pages.administration.organization-management.unit.modal.add-modal')
    @include('pages.administration.organization-management.unit.modal.update-modal')

@endsection

@push('scripts')
    <script>
        const getUnitUrl = "{{ route('organization-management.updateunit.ajax', ['userId' => '__id__']) }}";
        const deleteUnitUrl = "{{ route('organization-management.deleteunit.ajax', ['userId' => '__id__']) }}";
    </script>
    <script>
        ...
        $(document).ready(function () {
          ...
          $('#addUnitForm').on('submit', function(e) {
                ...
                $('#addUnitForm input, #addUnitForm select').removeClass('is-invalid');
                $('.invalid-feedback').remove();

                let name = $('#name').val();

                $.ajax({
                    url: '{{ route("organization-management.unit.store") }}',
                    method: 'POST',
                    data: {
                        name: name
                    },
                    ...
                    success: function(response) {
                        Swal.fire({
                            ...
                            $('#tableunit').DataTable().ajax.reload(null, false);
                        });
                    },
                    error: function(xhr) {
                        if (xhr.status === 422) {
        ...
        $(document).on('click', '[data-bs-target="#updateUnitModal"]', function () {
            let button = $(this);

            $('#name_update').val(button.data('name'));;
            $('#updateUnitForm').data('user-id', button.data('id'));
        ...
        $(document).ready(function () {
            ...
            $('#updateUnitForm').on('submit', function(e) {
                ...
                let url = getUnitUrl.replace('__id__', userId);
                ...
                let formData = {
                 ...
                 $.ajax({
                    ...
                    error: function(xhr) {
                        ...
                        for (let field in errors) {
                            let input = $('#updateUnitForm').find(`[name="${field}"]`);
                            input.addClass('is-invalid');
                            input.after(`<div class="invalid-feedback">${errors[field][0]}</div>`);
                        }
        ...
        $(document).ready(function () {
            ...
            $(document).on('click', '.btn-delete', function () {
                ...
                let url = deleteUnitUrl.replace('__id__', userId); 

                Swal.fire({
                    title: 'Are you sure?',
                    text: 'This action will permanently delete the Unit!',
                    icon: 'warning',
